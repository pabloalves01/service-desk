  ...
  <div class="p-14 flex flex-col gap-8">
    ...
    <div class="bg-zinc-950 border border-zinc-700 rounded-lg p-6 shadow-lg space-y-6">
      ...
      <div class="flex gap-4 pb-6 border-b border-zinc-700">
        <Button 
          class="bg-zinc-800 hover:bg-zinc-700 text-white" 
          @click="redirectToUploadPage(evento.codigo)"
        >
        ...
        <Button 
          class="bg-zinc-800 hover:bg-zinc-700 text-white" 
          @click="redirectToSlideshow(evento.codigo)"
        >
          <template #icon>
      ...
      <div class="space-y-4">
        ...
        <!-- Grid de fotos -->
        <div v-if="filteredFotos.length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          <div 
            v-for="(foto, index) in filteredFotos" 
            :key="index"
...
export default {
  ...
  methods: {
    ...
    async aprovarFoto(id) {
      try {
            // await axiosInstance.put(`/photos/${id}/approve`);
        
        // Simulação de aprovação (remover quando implementar a API)
    ...
    async rejeitarFoto(id) {
      try {
        // await axiosInstance.put(`/photos/${id}/reject`);
        
        // Simulação de rejeição (remover quando implementar a API)
        const foto = this.evento.fotos.find(f => f.id === id);
    ...
    async excluirFoto(id) {
      try {
        ...
        // Simulação de exclusão (remover quando implementar a API)
